...
// void Play_brithday(){ //生日歌
//   for (int thisNote=0; thisNote<(sizeof(brithday_note)/sizeof(int)); thisNote++){
//     int noteduration=1000/brithday_duration[thisNote];
//     tone(8,brithday_note[thisNote], noteduration);
...
void setup() {
pinMode(2, INPUT_PULLUP); //Button 1 with internal pull up当未与地线连接时为高电压，与地线连接时是低电压
...
void loop() {
    ...
    if (digitalRead(2)==LOW)
      { Serial.println("Selected -> '加勒比海盗' ");  Play_Pirates();  }
    if (digitalRead(3)==LOW)
      { Serial.println("Selected -> '疯狂青蛙' ");  Play_CrazyFrog();  }
    if (digitalRead(4)==LOW)
      { Serial.println("Selected -> '超级马里奥' ");  Play_MarioUW();  }
    if (digitalRead(5)==LOW)
      { Serial.println("Selected -> '泰坦尼克号' ");  Play_Titanic();  }
}
